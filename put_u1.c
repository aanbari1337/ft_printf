...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/18 23:18:00 by aanbari           #+#    #+#             */
/*   Updated: 2020/01/19 21:09:56 by aanbari          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	ft_u_p_q(char *str, t_tab *tab)
{
	if (str[0] == '-')
	{
...
static int	ft_u_p_m(char *str, t_tab *tab)
{
	if (str[0] == '-')
	{
		tab->len += write(1, "-", 1);
...
int			put_u1(char *str, t_tab *tab)
{
	if (tab->treat[0] == 'q' || tab->treat[0] == '0')
		return (ft_u_p_q(str, tab));
	if (tab->treat[0] == '-')
		return (ft_u_p_m(str, tab));
	return (tab->len);
}
